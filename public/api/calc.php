...
if (!is_array($config)) {
    http_response_code(500);
    echo json_encode(['error' => 'Configuration file is invalid']);
    exit;
}
...
if (!in_array($input['load_port'], $loadPorts, true)) {
    http_response_code(400);
    echo json_encode(['error' => 'Load port is not in the allowed list']);
    exit;
}

if (!in_array($input['discharge_port'], $dischargePorts, true)) {
    http_response_code(400);
    echo json_encode(['error' => 'Discharge port is not in the allowed list']);
    exit;
}
...
if (!array_key_exists($cargoName, $cargoMap)) {
    http_response_code(400);
    echo json_encode(['error' => 'Cargo is not in the allowed list']);
    exit;
}
